...
DramCtrl::DramCtrl(sc_core::sc_module_name name) : sc_module(name) {
    ...
    //mem_interconnect_socket_s2.register_b_transport(this, &DramCtrl::b_transport2);  
...
void DramCtrl::b_transport2(pl_t &pl, sc_core::sc_time &offset) // for socket interconnect_socket_s2
{
  tlm::tlm_command cmd = pl.get_command();
  sc_dt::uint64 addr = pl.get_address();
  unsigned int len = pl.get_data_length();
