...
def filter_image(im, filter): #this function will go through X gradient filter and the Y gradient filter
...
def get_block_descriptor(ori_histo, block_size):
    ...
    while x_window + block_size <= height:
        while y_window + block_size <= width:
            ...
            #print("concatanatedhist: ", concatednatedHist)
            #print("\nSize of histNorm: ", histNorm)
...
def face_recognition(I_target, I_template):
    ...
    extract_hog_c.argtypes = [
        ...
        np.ctypeslib.ndpointer(dtype=np.float64, shape = (((int(box_h/8)) - 1)*(int(box_w/8)-1)*24))
    ]
    ...
    #pyhog = extract_hog(I_template)
    #print("PY HOG: ", pyhog[1000:1020])
    template_HOG = np.float64(np.zeros(((int(box_h/8)) - 1)*((int(box_w/8)) - 1)*24))
    I_template2 = I_template
    I_template2 = np.float64(I_template2)
    extract_hog_c(I_template2, template_HOG)
    #template_HOG = list(template_HOG)
    #print("C HOG: ", template_HOG[1000:1020])
    #template_HOG = extract_hog(I_template)
    #template_HOG = template_HOG.tolist()
    #template_HOG = template_HOG.value
    #template_HOG = [round(num, 6) for num in template_HOG]
    template_HOG = template_HOG - np.mean(template_HOG) # Normalize
    template_HOG_norm = np.linalg.norm(template_HOG)
    x = 0
    y = 0
    img_HOG = np.float64(np.zeros(((int(box_h/8)) - 1)*((int(box_w/8)) - 1)*24))
    print("test")
    ...
    while x + box_h <= img_h:
        ...
        while y + box_w <= img_w:   
            ...
            #init_function(box_h, box_w)
            img_window = np.float64(img_window)
            extract_hog_c(img_window, img_HOG)
            #img_HOG = list(img_HOG)
            #img_HOG = [round(num, 6) for num in img_HOG]
    ...
    while thresholded_boxes != []:
        ...
        for box in thresholded_boxes:
            if box_iou(currBox, box, box_w) >= 0.4:
                toRemove.append(box)
...
def face_recognition_range(I_target, step):
    ...
    return found_faces, np.int32(real_face)
