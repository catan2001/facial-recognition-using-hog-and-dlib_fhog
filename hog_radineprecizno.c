    ...
    for (int i=0; i<ROWS; ++i){
        for (int j=0; j<COLS; ++j){
            ...
            for(int k=0; k<9; ++k){
                ...
                //im_filtered[i][j]=((512+(int)im_filtered[i][j])%256);
    ...
    for(int i = BLOCK_SIZE; i <= HEIGHT; i+=BLOCK_SIZE){
        for(int j = BLOCK_SIZE; j <= WIDTH; j+=BLOCK_SIZE){
            ...
            for(int k=0; k<HIST_SIZE; ++k) {
                concatednatedHist[k]=(double)ori_histo[i-BLOCK_SIZE+(int)k/12][j-BLOCK_SIZE+((int)k/6)%2][(k%6)];
