...
#import ctypes 
#lib = ctypes.CDLL("C:/Users/User/Documents/ml_projekat/HOG_FACE_DET/radni_folder/hog.so")
#so_file = "C:/Users/User/Documents/ml_projekat/HOG_FACE_DET/c_extract_hog/hog.so"
#c_hog = CDLL(so_file)
...
def get_gradient(im_dx, im_dy):
    ...
    grad_mag = np.zeros_like(im_dx,dtype=np.single) #dunno why they didn't just use np.zeros but it's an empty matrix the same size and shape as im_dx
    grad_angle = np.zeros_like(im_dx,dtype=np.single) #same goes for this young fella
    for i in range(height):
        grad_mag[i] = [np.linalg.norm([dX,dY]) for (dX,dY) in zip(im_dx[i],im_dy[i])] #(dX^2+dY^2)^1/2 zip pairs up im_dx[i][j] with im_dy[i][j] in a single array
                                                                                      #((im_dx[i][0], im_dy[i][0]),...,(im_dx[i][COLS], im_dy[i][COLS]))
        for j in range(width):
            dX = im_dx[i][j]
            dY = im_dy[i][j]
            if abs(dX) > 0.00001:
...
def build_histogram(grad_mag, grad_angle, cell_size):
    ...
    while (x_corner + cell_size) <= height:
        while (y_corner + cell_size) <= width:
            hist = np.zeros((6), dtype=np.single)
...
def get_block_descriptor(ori_histo, block_size):
    ...
    ori_histo_normalized = np.zeros(((height-(block_size-1)), (width-(block_size-1)), (6*(block_size**2))), dtype=np.single)
    while x_window + block_size <= height:
        while y_window + block_size <= width:
            ...
            histNorm = np.sum(np.square(concatednatedHist)) + 0.001
            #print("suma: ", (x_window), (y_window), histNorm, "\n")
            histNorm = np.sqrt(histNorm)
            #print("korijen: ", (x_window), (y_window), histNorm, "\n")
...
def extract_hog(im):
    ...
    im = im.astype(dtype =np.single) / 255.0
...
def face_recognition(I_target, I_template):
    ...
    #c_hog.extract_hog(I_template, template_HOG)
    ...
    while x + box_h <= img_h:
        ...
        while y + box_w <= img_w:
            ...
            #c_hog.extract_hog(img_window, img_HOG)
...
def txt_to_img(name_txt, img):
    ...
    cv2.imwrite(img, np.array(array))
...
def matrix_to_txt(name_txt, matrix):
    ...
    for i in range(0, matrix.shape[0], 1):
        for j in range(0, matrix.shape[1], 1):
            f.write('%.16f ' % matrix[i][j]) 
        f.write('\n') 
    f.close()

def cube_to_txt(name_txt, cube):
    f = open(name_txt, "w")
    for i in range(0, cube.shape[0], 1):
        for j in range(0, cube.shape[1], 1):
            for k in range(0, cube.shape[2], 1):
                f.write('%f ' % cube[i][j][k]) 
            f.write('\n') 
        f.write('\n') 
    f.close()
