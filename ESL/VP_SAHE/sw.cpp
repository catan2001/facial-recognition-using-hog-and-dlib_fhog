...
void SW::extract_hog(int rows, int cols, double *im, double *hog) { 
    ...
    write_hard(ADDR_SAHE6, sahe6, offset);
    //SAHE5:
    u32_t sahe5 = (cols+2)*(rows+2);
    
    write_hard(ADDR_SAHE5, sahe5, offset);
    ...
    //SAHE2:
    u6_t cycle_num_out = floor(BRAM_WIDTH/cols);
    u6_t cycle_num_in = floor(BRAM_WIDTH/(cols+2));
    u10_t effective_row_limit = floor((rows+2)/PTS_PER_COL)*PTS_PER_COL + accumulated_loss;
    u8_t width_4 = floor((cols+2)/4);
    
    u32_t sahe2 = (cycle_num_out << 24) | (cycle_num_in << 18) | (effective_row_limit << 8) | (width_4);

    write_hard(ADDR_SAHE2, sahe2, offset);

    //SAHE1:
    u9_t width_2 = (floor(cols/2)+1);
    u11_t picture_height = rows+2;
    u10_t picture_width = cols+2;
    u1_t start = 1;
