...
void SW::process_img(void){
    ...
    face_recognition_range(&gray[0], 20, 150);
...
void SW::extract_hog(int rows, int cols, double *im, double *hog) { 
    ...
    //write_dram(DRAM_LOW_ADDR | ADDR_COLS | cols, 0, offset); 
    //cout << "hello" << endl;
    //write_dram(DRAM_LOW_ADDR | ADDR_ROWS | rows, 0, offset);

    //write image to DRAM:
    for(int i=0; i<rows+2; ++i){
      for(int j=0; j<cols+2; ++j){
        //cout<<padded_img[i][j]<<" ";
        write_dram(DRAM_LOW_ADDR | WRITE_IMG | cols ,padded_img[i][j], offset);
      }
    }
    offset = temp;
    ...
    while(!read_hard(ADDR_STATUS, offset));
    write_hard(ADDR_RESET, 0, offset);
    ...
    write_hard(ADDR_BRAM_HEIGHT, 16, offset);
    ...
    //axis signals:
    write_hard(ADDR_HP0_STRB_IN, 0xFF, offset);
    write_hard(ADDR_HP1_STRB_IN, 0xFF, offset);
    write_hard(ADDR_HP0_READY_OUT, 1, offset);
    write_hard(ADDR_HP1_READY_OUT, 1, offset);
    ...
    while(!ready){
        ...
        //DRAM -> BRAM
        ready_hp0_in = read_hard(ADDR_HP0_READY_IN , offset);
        ready_hp1_in = read_hard(ADDR_HP1_READY_IN , offset);
        
        if(ready_hp0_in == 1 & ready_hp1_in == 1){
            //if the hw is ready write 4 pixels for every clk period: !!!!!!!!!!
            //write_dram(data_in1 and data_in2);
            write_dram(DRAM_LOW_ADDR | ADDR_DATA_IN, 0, offset);
	    write_hard(ADDR_HP0_VALID_IN, 1, offset);
	    write_hard(ADDR_HP1_VALID_IN, 1, offset);
           
            cnt++;
            if(cnt == ((cols+2)*(rows+2)/8 - 1)){
                write_hard(ADDR_HP0_LAST_IN, 1, offset);
                write_hard(ADDR_HP1_LAST_IN, 1, offset);
            }
        }
	//printf("cnt = %d", cnt);
        //BRAM -> DRAM
        valid_hp0_out = read_hard(ADDR_HP0_VALID_OUT, offset);
        valid_hp1_out = read_hard(ADDR_HP1_VALID_OUT, offset);
        strb_hp0_out = read_hard(ADDR_HP0_STRB_OUT, offset);
        strb_hp1_out = read_hard(ADDR_HP1_STRB_OUT, offset);

        if(valid_hp0_out == 1 & valid_hp1_out == 1 & strb_hp0_out == 0xFF & strb_hp1_out == 0xFF){
            //if the hw is ready write 4 pixels for every clk period: !!!!!!!!!!!!!!!
            write_dram(DRAM_LOW_ADDR | ADDR_DATA_OUT, 0, offset);
            write_hard(ADDR_HP0_LAST_IN, 0, offset);
            write_hard(ADDR_HP1_LAST_IN, 0, offset);
        }
    ...
    //cout << i+1 << ":" << endl; 
      for(int j = 0; j<cols; ++j){
        read_dram(READ_DX, matrix_im_filtered_x[i][j], offset);
        read_dram(READ_DY, matrix_im_filtered_y[i][j], offset);
    ...
    write_input_txt(matrix_im_filtered_x, dx);
    write_input_txt(matrix_im_filtered_y, dy);
...
{
    pl_t pl;
    //LEN_IN_BYTES*2 -> 32bits per SAHE
    
    unsigned char buf[LEN_IN_BYTES*2];
    //unsigned char buf[LEN_IN_BYTES];

    int32_to_uchar(buf, val); // NOTE: only for int...
    //int_to_uchar(buf, val); // NOTE: only for int...
    
    pl.set_address(addr | HARD_BASE_ADDR);

    pl.set_data_length(LEN_IN_BYTES*2);
    //pl.set_data_length(LEN_IN_BYTES);
