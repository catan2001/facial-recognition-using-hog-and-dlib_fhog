...
# 68 point face model
PREDICTOR_PATH = "/home/catic/Documents/project/final_version/copy/shape_predictor_68_face_landmarks"
# face detector
...
def landmark_detector_dlib(image) :
    ...
    #image = resize(image, 200/max_dim)
    ...
    #bounding_boxes = face_recognition_range(gray, 1, 75)
    #print("bounding boxes from HOG:", bounding_boxes)
    
    #visualize_face_detection(image, np.array(bounding_boxes))
...
def landmark_detector_hog(image) :
    ...
    max_dim = max(image.shape[0], image.shape[1])
    image = hog_temp.resize(image, 200/max_dim)
    ...
    bounding_boxes, real_face, time_rr, n_fr, n_eh, t_ngs, t_fi, t_gg, t_bh, t_gbd, t_f = hog_temp.face_recognition_range(gray, 10)
    #print("bounding boxes from HOG:", bounding_boxes)
    ...
    if face.is_empty():
        ...
        for n in range(17, 68): # NIJE POTREBAN OKVIR FACE!
                ...
                landmarks[n] = x, y
    time_end = time()
    time_landmarks = time_end - time_start
    return landmarks, time_landmarks, time_rr, n_fr, n_eh, t_ngs, t_fi, t_gg, t_bh, t_gbd, t_f    
