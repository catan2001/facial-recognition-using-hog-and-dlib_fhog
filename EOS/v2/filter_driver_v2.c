...
ssize_t filter_write(struct file *pfile, const char __user *buf, size_t length, loff_t *off)
{
	...
	{
		dma_simple_write(tx1_phy_buffer, val, dma1->base_addr + MM2S_DMACR_OFFSET); // helper function, defined later
		dma_simple_read(rx1_phy_buffer, pos, dma1->base_addr + MM2S_DMACR_OFFSET);
